 ...
 */
function mapJavaHibernateUsing(table_name, sql_vars, non_public = false) {
	const template  = TemplateLoader.loadFromFile(__dirname.replace(/\\/gi, '/') + '/template/java-hibernate-and-afquery.template');
	...
	meta_java['attributes'].forEach((attr, index) => {
		...
		attrs += indent(1, `@Column(name = "${attr.var_name}")\n`)
		// hibernate -- afquery
