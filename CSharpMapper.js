 ...
 */
function mapCsharpUsing(table_name, sql_vars, non_public = false) {
	const template  = TemplateLoader.loadFromFile(__dirname.replace(/\\/gi, '/') + '/template/csharp.template');
	...
	const meta_csharp = generator.extract();
	...
	let enum_var = [];
	meta_csharp['attributes'].forEach(attr => {
...
function mapCsharpFrom( sql_table, non_public = false ) {
	let input = SQLParser.parse(sql_table);
	return mapCsharpUsing(
		input.table_name,
		input.attributes,
		non_public
	);
